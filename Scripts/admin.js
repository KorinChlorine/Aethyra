...
const dataStore = {
  continents: {
    Africa: [
      {
        countryName: "Egypt",
        countryPhoto: "/Assets/Destinations/sample-country.jpg",
      },
    ],
    Asia: [],
    Europe: [],
...
async function updateDashboard() {
  ...
  ).innerHTML = `${countries.length}`;
  document.getElementById(
    "dashboard-spot-count"
  ).innerHTML = `${touristSpots.length}`;
}
...
async function selectContinent(name, continent_id) {
  ...
  document
    .getElementById("continent-table-wrapper")
    .scrollIntoView({ behavior: "smooth", block: "center" }); //smooth scroll to table
...
function clearContinentSelection() {
  ...
  document.getElementById("continent-country-form-wrapper").style.display =
    "none";
}
...
function renderContinentTable(countries, continent_id) {
  ...
  countries.forEach((country) => {
    ...
    editButton.addEventListener("click", () => {
      document.getElementById("continent-country-form-title").textContent =
        "Edit Country";
...
document
  .getElementById("continent-country-form")
  .addEventListener("submit", (e) => {
    e.preventDefault();
    const formData = new FormData(e.target); //gets the value of the form before sending to backend

    fetch("../Backend/manageCountry.php", {
      // connect to backend and pass form value
      method: "POST",
      body: formData,
    })
      .then((res) => res.text())
...
function removeCountry(country) {
  ...
  fetch("../Backend/removeCountry.php", {
    // connect to backend and pass country_id to delete
    method: "POST",
...
async function openCountriesForContinent(name, continent_id) {
  ...
  document.getElementById("countries-welcome-main").textContent =
    name.toUpperCase();
...
async function renderCountriesDetailList(countriesList) {
  ...
  for (const country of countries) {
    // loop through countries and dynamically creates DOM
    ...
    for (const [cityIndex, city] of citiesArray.entries()) {
      ...
      cityCard.innerHTML = `
        ...
        city.city_id
                                ...
                                <button type="button" class="deleteCity-button" onclick="removeCity(${
                                  city.city_id
                                })"> 
                                    Delete
                                </button>
                            ...
                            <button type="button" class="country-arrow-btn city-arrow-btn" onclick="toggleCityBlock('${key}', ${
        city.city_id
      })">
...
function buildCitySectionHtml(
  ...
  const labelId =
    "city-section-name-" + countryKey + "-" + cityIndex + "-" + sectionType;
  const photoId =
    "city-section-photo-" + countryKey + "-" + cityIndex + "-" + sectionType;
  const dotsId =
    "city-section-dots-" + countryKey + "-" + cityIndex + "-" + sectionType;
  const extraBlockId =
    "city-section-extra-" + countryKey + "-" + cityIndex + "-" + sectionType;
                ...
                <section class="city-section-block" data-section-type="${sectionType}">
                    ...
                    <div class="city-section-body">
                        ...
                        <div class="city-photo-wrapper">
                            <img id="${photoId}" src="${item.image}" alt="${
    item.name
  }">
                        </div>
                        <button type="button" class="circle-arrow-btn" onclick="stepCityItem('${countryKey}', ${cityIndex}, '${sectionType}', 1)">
                    ...
                    <footer class="city-section-footer">
                        <button type="button" class="add-button" onclick="
                             ${
                               sectionType === "spots"
                                 ? `openSpotsForm('add', '${countryKey}', ${cityIndex}, '${sectionType}')` //opens spots form
                                 : sectionType === "foods"
                                 ? `openFoodForm('add', '${countryKey}', ${cityIndex}, '${sectionType}')` //opens food form
                                 : sectionType === "activities"
                                 ? `openActivityForm('add', '${countryKey}', ${cityIndex}, '${sectionType}')`
                                 : "" //opens activities form
                             }
                        ">
                            ADD
                        </button>
                        <button type="button" class="edit-button" onclick="
                            ${
                              sectionType === "spots"
                                ? `openSpotsForm('edit', '${countryKey}', ${cityIndex}, '${sectionType}')`
                                : sectionType === "foods"
                                ? `openFoodForm('edit', '${countryKey}', ${cityIndex}, '${sectionType}')`
                                : sectionType === "activities"
                                ? `openActivityForm('edit', '${countryKey}', ${cityIndex}, '${sectionType}')`
                                : ""
                            }
                        ">
                            EDIT
...
function openCityForm(countryKey, cityIndex, city_id, country_id) {
  ...
  const cityData =
    cityIndex !== null && window.cityRuntime?.[countryKey]
  ...
  // Title
  document.getElementById("city-form-title").textContent = cityData
    ? "Edit City"
    : "Add City";
...
function updateDots(key, cityIndex, sectionType, total, currentIndex) {
  const dotsId =
    "city-section-dots-" + key + "-" + cityIndex + "-" + sectionType;
...
function stepCityItem(key, cityIndex, sectionType, delta) {
  ...
  document.getElementById(photoId).src =
    item.image || "../Assets/Destinations/sample-country.jpg";
...
function configureItemExtraFields(sectionType) {
  ...
  if (sectionType === "spots") {
    ...
    extra2.placeholder =
      "Practical details (best time to visit, entrance info, tips)";
  } else if (sectionType === "foods") {
    extra1Label.textContent = "About";
    extra1.placeholder =
      "Short description of this delicacy (taste, ingredients, significance)";
    extra2Label.textContent = "Location";
    extra2.placeholder =
      "Where to find it (markets, streets, recommended shops)";
  } else if (sectionType === "activities") {
...
function openSpotsForm(mode, key, cityIndex, sectionType) {
  ...
  const currentIndex =
    arr.length && mode === "edit" ? idxObj[sectionType] || 0 : -1;
  ...
  if (mode === "edit" && arr.length) {
    ...
    document.getElementById("item-extra-1").value =
      item.history || item.extra1 || "";
    document.getElementById("item-extra-2").value =
      item.details || item.extra2 || "";
...
function openFoodForm(mode, key, cityIndex, sectionType) {
  ...
  if (mode === "edit" && arr.length && arr[currentIndex]) {
    ...
    document.getElementById("food-city-id").value =
      item.city_id || cityData.city_id; //city_id to pass to db
...
function openActivityForm(mode, key, cityIndex, sectionType) {
  ...
  if (mode === "edit" && arr.length && arr[currentIndex]) {
    ...
    document.getElementById("activity-city-id").value =
      item.city_id || cityData.city_id;
...
function deleteCurrentCityItem(key, cityIndex, sectionType) {
  ...
  fetch(backendUrl, {
    ...
    .then((msg) => {
      ...
      idxObj[sectionType] =
        arr.length === 0 ? 0 : Math.min(currentIndex, arr.length - 1);
...
async function renderTopDestinations() {
  ...
  if (!top.length) {
    listEl.innerHTML = '<p class="top-empty">No visit records yet.</p>';
    return;
  }
...
async function loadUsersFromDatabase() {
  try {
    const res = await fetch("../Backend/getAllUsers.php?_cache=" + Date.now());
    const data = await res.json();
...
function renderUsersTable() {
  ...
  if (usersData.length === 0) {
    const tr = document.createElement("tr");
    tr.innerHTML =
      '<td colspan="10" style="text-align: center; padding: 20px;">No users found in database</td>';
    tbody.appendChild(tr);
    return;
  }

  // Build table headers dynamically from first user's keys (excluding userID)
  const headers = Object.keys(usersData[0]).filter((key) => key !== "userID");
...
function editUser(index) {
  ...
  document.getElementById("user-middleName").value = u.middleName || "";
...
function deleteUser(index) {
  ...
  fetch("../Backend/deleteUser.php", { method: "POST", body: fd })
    .then((res) => res.json())
    .then((data) => {
      if (data.success) {
...
document.getElementById("pending-prev-btn").addEventListener("click", () => {
  ...
  pendingIndex =
    (pendingIndex - 1 + pendingReviews.length) % pendingReviews.length;
  renderPendingReviews();
});
...
document.getElementById("approved-prev-btn").addEventListener("click", () => {
  ...
  approvedIndex =
    (approvedIndex - 1 + approvedReviews.length) % approvedReviews.length;
  renderApprovedReviews();
});
