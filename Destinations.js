...
function showCities(cities, page, continentName, countryName) {
  ...
  card.className = "col-sm-6 col-md-4 col-lg-3";
    ...
    seeMoreBtn.addEventListener("click", (e) => {
      e.stopPropagation();
      window.location.href = `DestinationPage.html?continent=${encodeURIComponent(continentName)}&country=${encodeURIComponent(countryName)}&place=${city.id}`;
    });
...
if (window.innerWidth < 600) {
  ...
  let touches = [], startTap = null;
  let start = {}, pinchStartDist = 0, pinchStartVB = [];
  ...
  _SVG.addEventListener('touchstart', e => {
    ...
    if (touches.length === 1) {
      startTap = { x: touches[0].clientX, y: touches[0].clientY, time: Date.now() };
      start = { x: touches[0].clientX, y: touches[0].clientY, vbX: VB[0], vbY: VB[1] };
      nav = { act: 'move' };
  ...
  _SVG.addEventListener('touchmove', e => {
    ...
    if (touches.length === 1) {
      ...
      const moveY = ((start.y - touches[0].clientY) / _SVG.clientHeight) * VB[3];
